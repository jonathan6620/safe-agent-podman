...
export function copyHostCredentials(name, { maxAttempts = 5, intervalMs = 500 } = {}) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    try {
      ...
      execFileSync("podman", ["exec", name, "chown", "vscode:vscode", "/home/vscode/.claude/.credentials.json"], {
        stdio: ["ignore", "ignore", "pipe"],
      });
      return;
    } catch (err) {
...
export function copyHostConfig(name, { maxAttempts = 5, intervalMs = 500 } = {}) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    try {
      ...
      execFileSync("podman", ["exec", name, "chown", "vscode:vscode", "/home/vscode/.claude.json"], {
        stdio: ["ignore", "ignore", "pipe"],
      });
      return; // success
    } catch (err) {
